...
class xception(nn.Module):
    def __init__(self, num_classes=2, pretrained='imagenet', escape=''):
        ...
        self.seq.append(('logits',
                         [self.relu4, lambda x: F.adaptive_avg_pool2d(x, (1, 1)), lambda x: x.view(x.size(0), -1),
                          self.last_linear]))
        if pretrained == 'imagenet':
            self.load_state_dict(
                model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/xception-43020ad28.pth'), strict=False)
        elif pretrained:
