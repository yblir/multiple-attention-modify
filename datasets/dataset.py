...
class DeepfakeDataset(Dataset):
    ...
    def __init__(self, phase='train', datalabel='', resize=(320, 320), imgs_per_video=30, min_frames=0, \
                 normalize=dict(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]), frame_interval=10, max_frames=300,
                 augment='augment0'):
        ...
        if 'ff-5' in self.datalabel:
            ...
            self.dataset += FF_dataset(self.datalabel.split('-')[1], self.datalabel.split('-')[2], phase) + FF_dataset(
                    "Origin", self.datalabel.split('-')[2], phase)
