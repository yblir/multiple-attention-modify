...
import cv2
from utils import dist_average, ACC, compute_metrics, gather_tensor
...
def main_worker(config):
    ...
    for epoch in range(start_epoch, config.epochs):
        ...
        train_loss_value, train_real_acc, train_fake_acc = run(epoch, world_size, data_loader=train_loader,
                                                               ...
                                                               config=config,
                                                               AG=AG, phase='train')
        val_loss_value, val_real_acc, val_fake_acc = run(epoch, world_size, data_loader=validate_loader,
                                                         net=net,
                                                         optimizer=optimizer, local_rank=local_rank,
                                                         config=config,
                                                         AG=AG, phase='valid')
        ...
        if local_rank == 0:
            ...
            checkpoints_save_name = f"Epoch{epoch}_real_acc{val_real_acc}_fake_acc{val_fake_acc}_loss{val_loss_value}.pth"
            # 写入tensorboard日志
...
def run(epoch, world_size, data_loader, net, optimizer, local_rank, config, AG=None, phase='train'):
    # if local_rank == 0:
    #     print('start ', phase)
    if config.AGDA_loss_weight == 0:
        AG = None
    ...
    # 合并分布式结果
    gather_out_list, gather_label_list = [], []
    ...
    for i, datas in enumerate(data_loader):
        ...
        if phase == 'train':
            ...
            optimizer.step()
            optimizer.zero_grad()
        gather_out_list.extend(loss_pack['ensemble_logits'])
        gather_label_list.extend(y)
        ...
        with torch.no_grad():
            ...
            if config.feature_layer == 'logits':
                ...
                acc, real_acc, fake_acc, real_cnt, fake_cnt = compute_metrics(loss_pack['ensemble_logit'], y)
        ...
        if local_rank == 0 and i % 100 == 0:
            ...
            logger.info(
                    ...
                    f"real_acc_{real_cnt}_{round(float(real_acc), 4) if real_cnt != 0 else 'NAN'}, "
                    f"fake_acc_{fake_cnt}_{round(float(fake_acc), 4) if fake_cnt != 0 else 'NAN'}, "
                    f"loss_{round(float(batch_loss.detach().cpu().numpy()), 5)}"
            )
        # for i in record_list:
        #     recorder[i].step(loss_pack[i])
    # if phase == "test" or phase == "val":
    gather_outs = gather_tensor(gather_out_list, world_size, to_numpy=False)
    gather_label = gather_tensor(gather_label_list, world_size, to_numpy=False)

    _, real_acc, fake_acc, real_cnt, fake_cnt = compute_metrics(gather_outs, gather_label)
    return round(temp_total_loss / data_length, 5), round(float(real_acc), 4), round(float(fake_acc), 4)
