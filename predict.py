...
if __name__ == '__main__':
    ...
    imgs = video_preprocess(video_path)
    count = 0
    with torch.no_grad():
        for img in imgs:
            # input_data = torch.from_numpy(img)
            # input_data = input_data.unsqueeze(0).float()
            images = img.cuda().unsqueeze(0).float()
            # print(input_data.dtype)
            logits = model(images)
            # pred = torch.nn.functional.softmax(logits, dim=1)[:, 1]
            # print(pred)
            # break
            real_probs = torch.softmax(logits, dim=1)[:, 1].cpu().numpy()[0]
            label = "real" if real_probs < 0.5 else "fake"
            if label == "real":
                count += 1
            logger.info(f"predict -> {label}, real_probs: {real_probs}")
        logger.success(f"real count = {count}")
        # real_probs = round(real_probs, 4)
        # print(real_probs)
